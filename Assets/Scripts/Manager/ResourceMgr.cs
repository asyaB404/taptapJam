...
public class ResourceMgr : Singleton<ResourceMgr>
{
    ...
    GameObject ordinaryRefreshObjs, quickRefreshObjs;
    ...
    {//普通刷新
        if (ordinaryRefreshObjs == null)
        {
            if (RefreshObjs == null)
            {
                RefreshObjs = GameObject.Find("RefreshObjs");
            }
            if (RefreshObjs != null)
            {
                ordinaryRefreshObjs = RefreshObjs.transform.Find("ordinaryRefreshObjs").gameObject;
            }
            else
            {
                ordinaryRefreshObjs = GameObject.Find("RefreshObjs");
            }
        }
        if(ordinaryRefreshObjs!=null)for (int i = 0; i < ordinaryRefreshObjs.transform.childCount; i++)
        {
            ordinaryRefreshObjs.transform.GetChild(i).gameObject.SetActive(true);
        }
        QuickResource();
    }
    public void QuickResource()
    {//快速刷新
        if (quickRefreshObjs == null)
        {
            if (RefreshObjs == null)
            ...
            if (RefreshObjs != null)
            {
                quickRefreshObjs = RefreshObjs.transform.Find("quickRefreshObjs").gameObject;
            }
            else
            {
                quickRefreshObjs = GameObject.Find("quickRefreshObjs");
            }
            if(quickRefreshObjs!=null)for (int i = 0; i < quickRefreshObjs.transform.childCount; i++)
            {
